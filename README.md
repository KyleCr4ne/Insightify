...
В общем смысле функции модуля Insightify::LossGradients высчитывают градиент соответственной функции потерь в точке пространства соответствующей выбранному сэмплу из выборки. Таким образом эти функции являются вспомогательными для реализации оптимизаторов на основе стохастического градиентного спуска. 


Регуляризации при вычислении градиента используется для контроля за весами модели в процессе обучения. Данная реализация предполгает использование L1 или L2 регуляризации. Для этого, при указание регуляризатора в ходе вычисления градиента дополнительно минимизурется штраф для высокие показатели вектора весов. Таким образом функция потерь принимает вид:
- $Loss = Loss + w_{1}^{2} + .. + w_{n}^{2}$ при L2 регуляризации
- $Loss = Loss + |w_1| + .. + |w_n|$ при L1 регуляризации
